...
import task from '../../pages/content/referred.to.interloc.task_en.json';
import { VoidCase } from './void.case';
import { SendToAdmin } from './send.to.admin';


export class ReferredToInterloc extends BaseStep {
    ...
    async verifyTcwWithoutCaseAllocatorRoleCanViewReviewReferredToInterlocTask(caseId: string) {

        // Verify TCW can view the unassigned Referred by Interloc task
    ...
    async verifyTcwWithCaseAllocatorRoleCanViewAndAssignReferredToInterlocTask(caseId: string) {

        /* Login as Senior TCW with case allocator role and view the 
           unassigned Referred by Interloc task and assign it to another TCW */
    ...
    async verifyTcwAsAnAssignedUserForReferredToInterlocTaskCanViewAndCompleteTheTask(caseId: string) {

        // Login as TCW and view the unassigned Referred by Interloc task
        await this.loginUserWithCaseId(credentials.amTribunalCaseWorker, true, caseId);
        await this.homePage.navigateToTab('Tasks')
        await this.tasksTab.verifyTaskIsDisplayed(task.name);
        await this.tasksTab.verifyPageContentByKeyValue(task.name, 'Assigned to', task.assignedTo);
    ...
    async verifyReferredToInterlocTaskIsCancelledAutomaticallyWhenTheCaseIsVoid(caseId: string) {

        // Verify TCW with case allocator role can view the unassigned Referred by Interloc task
