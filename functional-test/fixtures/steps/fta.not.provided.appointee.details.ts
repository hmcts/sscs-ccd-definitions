...
import task from '../../pages/content/review.fta..not.provided.appointee.details_en.json';
import { VoidCase } from './void.case';
import { SendToAdmin } from './send.to.admin';


export class FtaNotProvidedAppointeeDetails extends BaseStep {
    ...
    async verifyTcwWithoutCaseAllocatorRoleCanViewReviewAbateFtaNotProvidedAppointeeDetailsTask(caseId: string) {

        // Verify TCW can view the unassigned Abate Fta Not Provided Appointee Details task
    ...
    async verifyTcwWithCaseAllocatorRoleCanViewAndAssignAbateFtaNotProvidedAppointeeDetailsTask(caseId: string) {

        /* Login as Senior TCW with case allocator role and view the 
           unassigned Abate Fta Not Provided Appointee Details task and assign it to another TCW */
    ...
    async verifyTcwAsAnAssignedUserForAbateFtaNotProvidedAppointeeDetailsTaskCanViewAndCompleteTheTask(caseId: string) {

        // Login as TCW and view the unassigned Abate Fta Not Provided Appointee Details task
        await this.loginUserWithCaseId(credentials.amTribunalCaseWorker, true, caseId);
        await this.homePage.navigateToTab('Tasks')
        await this.tasksTab.verifyTaskIsDisplayed(task.name);
        await this.tasksTab.verifyPageContentByKeyValue(task.name, 'Assigned to', task.assignedTo);
    ...
    async verifyAbateFtaNotProvidedAppointeeDetailsTaskIsCancelledAutomaticallyWhenTheCaseIsVoid(caseId: string) {

        // Verify TCW with case allocator role can view the unassigned Abate Fta Not Provided Appointee Details task
